...
[[ $# != 0 ]] && all_parameter=($(echo $@)) || {
    echo 'Err  !!! Useage: bash this_script.sh membercenter 13800008888@112233 18388880000@123456'
    exit 1
}
all_username_password=($(echo ${all_parameter[*]} | grep -oE "[0-9]{11}@[0-9]{6}" | sort -u | tr "\n" " "))

# 登录失败尝试修改以下这个appId的值为抓包获取的登录过的联通app,也可使用传入参数 appId@*************
appId=cb648e652626d4cb7d0ec3f706b4a33dc078c455510cfb83248a9eabe4b59289aa615c14b969a247577a268fcb290f9902f739ce70a064f8499e4f81b5f37c2b
...
function membercenter() {
    ...
    for ((i = 1; i <= 3; i++)); do
        [[ $i -gt 3 ]] && curl -sA "$UA" -b $workdir/cookie.SigninActivity --data "goldnumber=10&banrate=10&usernumberofjsp=$usernumberofjsp" https://m.client.10010.com/dailylottery/static/doubleball/duihuan >/dev/null
        sleep 1
    ...
    echo
    echo goldTotal：$(curl -X POST -sA "$UA" -b $workdir/cookie.SigninActivity -e "$Referer" "https://act.10010.com/SigninApp/signin/getGoldTotal?vesion=0.$(shuf -i 1234567890123456-9876543210654321 -n 1)")
    ...
    for ((i = 1; i <= 3; i++)); do
        curl -sA "$UA" -b $workdir/cookie.SigninActivity --data "goldnumber=10&banrate=30&usernumberofjsp=$usernumberofjsp" http://m.client.10010.com/dailylottery/static/integral/duihuan >/dev/null
        sleep 1
    ...
    # 每日领100定向积分
    echo
    curl -X POST -sA "$UA" -b $workdir/cookie --data "from=$(shuf -i 12345678901-98765432101 -n 1)" https://m.client.10010.com/welfare-mall-front/mobile/integral/gettheintegral/v1
    ...
    # 奖励积分
    echo
    curl -X POST -sA "$UA" -b $workdir/cookie.SigninActivity --data "methodType=signin" https://m.client.10010.com/producGame_signin
    ...
    echo
    curl -X POST -sA "$UA" -b $workdir/cookie.xybx --data "methodType=reward&deviceType=Android&clientVersion=$unicom_version&isVideo=N" https://m.client.10010.com/game_box
    echo
    curl -sA "$UA" -b $workdir/cookie.xybx --data "methodType=taskGetReward&deviceType=Android&clientVersion=$unicom_version&taskCenterId=98" https://m.client.10010.com/producGameTaskCenter
    echo
    curl -X POST -sA "$UA" -b $workdir/cookie.xybx --data "methodType=reward&deviceType=Android&clientVersion=$unicom_version&isVideo=Y" https://m.client.10010.com/game_box
    ...
    echo
    curl -X POST -sA "$UA" -b $workdir/cookie.wotree -c $workdir/cookie.wotree -e "$Referer" https://m.client.10010.com/mactivity/arbordayJson/arbor/3/0/3/grow.htm | grep -oE "addedValue\":[0-9]"

    # 获得流量
    for ((i = 1; i <= 3; i++)); do
        curl -X POST -sA "$UA" -b $workdir/cookie --data "stepflag=22" https://act.10010.com/SigninApp/mySignin/addFlow >/dev/null
        sleep 5
    ...
    # info
    echo
    echo membercenter accomplished.
}
...
function main() {
    ...
    for ((u = 0; u < ${#all_username_password[*]}; u++)); do
        ...
        userlogin && userlogin_ook[u]=$(echo ${username:0:3}****${username:7}) || {
            userlogin_err[u]=$(echo ${username:0:3}****${username:7})
            continue
        }
        membercenter
        liulactive
    ...
    echo
    echo $(date) ${userlogin_err[*]} ${#userlogin_err[*]} Failed. ${userlogin_ook[*]} ${#userlogin_ook[*]} Accomplished.
    # TG通知
    tgbotinfo
}
